...
def searchListing():
    ...
    if shortlisted_only:    
        if not shortlisted_ids:
            ...
            all_services = searchListingController.searchListing(name, email, service_name, min_price, max_price, type)
            services = [s for s in all_services if s['id'] in shortlisted_ids]
    else:
        services = searchListingController.searchListing(name, email, service_name, min_price, max_price, type)
        
    return render_template('browse_services.html', services=services, shortlisted_ids=shortlisted_ids, first_name=first_name)
